...
class AttackManager(Model):
    ...
    def token_gradients(self, input_ids, input_slice, target_slice, loss_slice):
        '''
        This function is used to calculate the gradients of the logits to adversial suffix.
        '''
        def net(one_hot):
        ...
        for i in range(control_cand.shape[0]):
            ...
            if filter_cand:
                if (decoded_str != curr_control and
                    len(self.tokenizer(decoded_str, add_special_tokens=False).input_ids) ==
                    len(control_cand[i])):
    ...
    def get_logits(
        ...
        if isinstance(test_controls[0], str):
            ...
            test_ids = [
                ms.tensor(self.tokenizer(control, add_special_tokens=False).input_ids[:max_len])
        ...
        del locs, test_ids
        logits = self.forward( input_ids=ids, attention_mask=attn_mask, batch_size=self.batch_size)
        del ids
        gc.collect()
        return logits
